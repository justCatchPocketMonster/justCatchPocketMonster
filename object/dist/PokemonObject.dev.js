...
var fonctionJs = require("../fonction/fonctionJs");
...
var valeurMaxRandom = variableGlobal.valeurMaxRandom + 1;

var savePokemonServer = require("../fonction/pokedexSaveServer");
/**
...
function pokemonChoice(type, gen, typeReal) {
  ...
  var pokemonDoSpawn = pokemonSelected[fonctionJs.getRandomInt(pokemonSelected.length)];
...
function pokemonSelect() {
  var randomTypePokemon = typeIntToStr(fonctionJs.getRandomInt(valeurMaxRandom));
  var randomGen = generationSelect();
  var randomType = typeSelect();
  var pokemonChoiced = {};

  while (pokemonChoiced["id"] === undefined) {
    var _randomTypePokemon = typeIntToStr(fonctionJs.getRandomInt(valeurMaxRandom));

    var _randomGen = generationSelect();

    var _randomType = typeSelect();

    pokemonChoiced = pokemonChoice(_randomTypePokemon, _randomGen, _randomType);
  }
...
function shinySelect(pokemon, idServer, message) {
  var tauxShiny = variableGlobal.tauxMaxShiny;
  var saveServer = savePokemonServer.getSave(idServer);
  ...
  var nbRandomShiny = fonctionJs.getRandomInt(tauxShiny);
...
module.exports = {
  ...
  getNamePokemon: getNamePokemon,
  shinySelect: shinySelect
};