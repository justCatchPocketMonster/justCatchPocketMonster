...
export class spawnHandler implements MenuHandler {
  ...
  getMenuStructure(): MenuOption {
    const availableChannels = this.guild
      ? this.getAvailableChannelsMenuOptions(this.guild)
      : undefined;
    const allowedChannels = this.guild
      ? this.getAllowedChannelsMenuOptions(this.guild)
      : undefined;
    ...
    const children: MenuOption[] = [
      {
        ...
        description: language(
          "adminSettingsSpawnAddDescription",
          this.server.settings.language,
        ),
        placeholder: language(
          "adminSettingsSpawnAddPlaceholder",
          this.server.settings.language,
        ),
      ...
      {
        label: language(
          "adminSettingsSpawnRemove",
          this.server.settings.language,
        ),
        value: "remove",
        description: language(
          "adminSettingsSpawnRemoveDescription",
          this.server.settings.language,
        ),
        placeholder: language(
          "adminSettingsSpawnRemovePlaceholder",
          this.server.settings.language,
        ),
  ...
  private getMainEmbed(): EmbedBuilder {
    ...
    this.server.channelAllowed.forEach((channelId: string) => {
      ...
      if (!channel) {
        channelsList.push(
          language("adminSettingsSpawnEmbedChannelDeleted", lang).replace(
            "{channelId}",
            channelId,
          ),
        );
  ...
  private getAvailableChannelsMenuOptions(guild: Guild): MenuOption[] {
    ...
    guild.channels.cache.forEach((channel) => {
      ...
      const categoryName =
        channel.parent?.name ||
        language("adminSettingsSpawnNoCategory", this.server.settings.language);
      const channelName = channel.name;
      const displayName = `${categoryName} - ${channelName}`;
      ...
      const description = language(
        "adminSettingsSpawnAddChannelDescription",
        this.server.settings.language,
    ...
    if (availableChannels.length === 0) {
      return [
        {
          label: language(
            "adminSettingsSpawnNoChannelsAvailable",
            this.server.settings.language,
          ),
          value: "no_channels",
          description: language(
            "adminSettingsSpawnNoChannelsAvailableDescription",
            this.server.settings.language,
          ),
        },
      ];
  ...
  private getAllowedChannelsMenuOptions(guild: Guild): MenuOption[] {
    ...
    this.server.channelAllowed.forEach((channelId: string) => {
      ...
      if (!channel) {
        const deletedLabel = language(
          "adminSettingsSpawnDeletedChannel",
          this.server.settings.language,
        ...
        allowedChannels.push({
          ...
          value: channelId,
          description: language(
            "adminSettingsSpawnDeletedChannelDescription",
            this.server.settings.language,
          ),
        });
        return;
      ...
      const description = language(
        "adminSettingsSpawnRemoveChannelDescription",
        this.server.settings.language,
      ).replace("{channelName}", channelName);
    ...
    if (allowedChannels.length === 0) {
      return [
        {
          label: language(
            "adminSettingsSpawnNoChannelsAllowed",
            this.server.settings.language,
          ),
          value: "no_channels",
          description: language(
            "adminSettingsSpawnNoChannelsAllowedDescription",
            this.server.settings.language,
          ),
        },
      ];
