...
describe('code command', () => {
    ...
    test('Activate shiny code', async () => {
        ...
        (interaction.options.getString as jest.Mock).mockImplementation((name: string) => {
            if (name === language("codeNameOptionString", "eng")) return 'testForShiny';
            return null;
        });
    ...
    test('Auto generated code spawn', async () => {
        ...
        (interaction.options.getString as jest.Mock).mockImplementation((name: string) => {
            if (name === language("codeNameOptionString", "eng")) return 'SPAWNS10000';
            return null;
        });
    ...
    test('Auto generated code catch', async () => {
        ...
        await updateStat(statGiven.version, statGiven);
        (interaction.options.getString as jest.Mock).mockImplementation((name: string) => {
            if (name === language("codeNameOptionString", "eng")) return 'SPAWNS5000';
            return null;
        });
    ...
    test('Code already used', async () => {
        ...
        expect(replyMock.mock.calls[0][0].content).toBe(language("codeAlreadyUsed", "eng"));
    });

    test('Code unknowed', async () => {
        // given
        (interaction.options.getString as jest.Mock).mockImplementation((name: string) => {
            if (name === language("codeNameOptionString", "eng")) return 'unknownCode';
            return null;
        });
