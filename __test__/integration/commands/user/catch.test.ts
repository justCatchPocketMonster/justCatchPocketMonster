...
describe('catch command', () => {
    ...
    test('Catch present pokemon', async () => {
        ...
        expect(replyMock).toHaveBeenCalledWith("Congratulations TestNick, you caught one Keunotor/Bidoof. ");
    ...
    test('Catch present pokemon shiny', async () => {
        ...
        expect(replyMock).toHaveBeenCalledTimes(1);
        expect(replyMock).toHaveBeenCalledWith("Congratulations TestNick, you caught one Keunotor/Bidoof:star:. ");
    ...
    test('Catch present pokemon but shiny event', async () => {
        ...
        expect(replyMock).toHaveBeenNthCalledWith(1,"Wait... But the Pokémon is painted! It is shiny!!!");
        expect(replyMock).toHaveBeenNthCalledWith(2,"Congratulations TestNick, you caught one Keunotor/Bidoof:star:. ");
        await checkCatchForAllSavePokemon(interaction, 1, 1);
    ...
    test('Catch present pokemon shiny but not shiny event', async () => {
        ...
        expect(replyMock).toHaveBeenCalledTimes(2);
        expect(replyMock).toHaveBeenNthCalledWith(1,"Wait... But the Pokémon is painted! It is not shiny! What a desappointment...");
        expect(replyMock).toHaveBeenNthCalledWith(2,"Congratulations TestNick, you caught one Keunotor/Bidoof. ");
        await checkCatchForAllSavePokemon(interaction, 1, 0);
    ...
    test('Failed catch name is wrong', async () => {
        ...
        expect(replyMock).toHaveBeenCalledTimes(1);
        expect(replyMock).toHaveBeenCalledWith("Sorry TestNick. The current Pokémon is not Badoof.");
        await checkCatchForAllSavePokemon(interaction, 0, 0);
...
async function checkCatchForAllSavePokemon(interaction: ChatInputCommandInteraction,countNormal: number, countShiny: number) {
    ...
    expect(userThen.savePokemon.getSaveOnePokemonFusedForm("399").normalCount).toBe(countNormal);
    expect(userThen.savePokemon.getSaveOnePokemonFusedForm("399").shinyCount).toBe(countShiny);
    expect(serverThen.savePokemon.getSaveOnePokemonFusedForm("399").normalCount).toBe(countNormal);
    expect(serverThen.savePokemon.getSaveOnePokemonFusedForm("399").shinyCount).toBe(countShiny);
    expect(statNowThen.savePokemonCatch.getSaveOnePokemonFusedForm("399").normalCount).toBe(countNormal);
    expect(statNowThen.savePokemonCatch.getSaveOnePokemonFusedForm("399").shinyCount).toBe(countShiny);
    expect(statGeneralThen.savePokemonCatch.getSaveOnePokemonFusedForm("399").normalCount).toBe(countNormal);
    expect(statGeneralThen.savePokemonCatch.getSaveOnePokemonFusedForm("399").shinyCount).toBe(countShiny);
}
