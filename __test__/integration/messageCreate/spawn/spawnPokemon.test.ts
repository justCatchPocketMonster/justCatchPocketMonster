...
describe('Spawn Pokemon', () => {
    ...
    afterEach(() => {
        ...
        const { generationSelect, raritySelect, typeSelect } =
            jest.requireActual("../../../../src/features/pokemon/selectPokemon");
        __deps.generationSelect = generationSelect;
        __deps.raritySelect = raritySelect;
        __deps.typeSelect = typeSelect;
    });
    const generationPossibility = Object.keys(valuePerGen).map(key => {return key})
    describe.each(generationPossibility)(`%s`, (generation) => {
        const typePossibility = Object.keys(valuePerType).map(key => {return key})
        describe.each(typePossibility)(`%s`, (type) => {
            const rarityPossibility = Object.keys(valuePerRarity).map(key => {return key})
            test.each(rarityPossibility)(`%s`,async (rarity) => {
                ...
                const pokemonWithSameData: pokemonDb[] = allPokemon.filter(
                    p => p.gen.toString() === generation && p.arrayType.includes(type) && p.rarity === rarity
                );
                ...
                expect(data?.embed.data.description).toBe("To catch it, do \"/catch [Pokémon's name]\".")
                const serverThen = await getServerById(message.guildId!);
                expect(serverThen.pokemonPresent[message.channelId].gen.toString()).toBe(generation);
                expect(serverThen.pokemonPresent[message.channelId].rarity).toBe(rarity);
                expect(serverThen.pokemonPresent[message.channelId].arrayType).toContain(type);
            });
        });
    });
