...
async function runTest(file: string): Promise<TestResult> {
  ...
  try {
    ...
    console.log(
      `   ${status === "success" ? "✅" : "❌"} Completed in ${formatTime(duration)}`,
    );
    ...
    return {
      ...
      status,
      error: hasFailures
        ? "Test failed - see Jest output for details"
        : undefined,
    };
  } catch (error: any) {
    const endTime = Date.now();
    ...
    console.log(
      `   ${isTimeout ? "⏱️" : "❌"} ${isTimeout ? "Timeout" : "Error"} after ${formatTime(duration)}`,
    );
  ...
  try {
    ...
    while ((match = filePattern.exec(content)) !== null) {
      ...
      const fileSection = content.substring(
        Math.max(0, content.indexOf(match[0]) - 500),
        content.indexOf(match[0]) + 1000,
      );
      ...
      if (timeMatch && statusMatch) {
        ...
        if (timeStr.includes("m")) {
          ...
          timeMs =
            (parseInt(minutes) * 60 + parseFloat(seconds.replace("s", ""))) *
            1000;
    ...
    console.log(
      "⚠️  Unable to load existing results, starting from the beginning",
    );
  }
